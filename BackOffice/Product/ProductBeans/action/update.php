...
// --- (Fungsi uploadGambar SAMA PERSIS) ---
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql_gambar = "";
    $gambar_url_new = "";
    $old_file_path = ""; // Variabel untuk menyimpan path file LAMA

    // --- LOGIKA UPLOAD GAMBAR BARU & HAPUS GAMBAR LAMA ---
    if (isset($_FILES['gambar_url']) && $_FILES['gambar_url']['error'] == 0) {

        // 1. Ambil path gambar LAMA dari DB (untuk dihapus nanti)
        ...
        if ($uploadResult['success']) {
            ...
            $sql_gambar = ", gambar_url = ?"; // Siapkan query SQL
        } else {
    ...
    try {
        ...
        // 8. Proses Varian (Update, Insert, Delete)
        ...
        for ($i = 0; $i < count($varian_ids); $i++) {
            ...
            $id_grind = (int)$varian_grinds[$i];
            $harga = (int)$varian_hargas[$i];
            $stok = (int)$varian_stoks[$i];
            if ($id_varian == 'new') {
                $stmt_insert_varian->bind_param("iiiii", $id_produk, $id_size, $id_grind, $harga, $stok);
                $stmt_insert_varian->execute();
            } else {
                $stmt_update_varian->bind_param("iiiii", $id_size, $id_grind, $harga, $stok, $id_varian);
        ...
        // 9. HAPUS FILE GAMBAR LAMA (JIKA ADA)
        ...
        $_SESSION['message'] = "Produk Biji Kopi berhasil diperbarui.";
