...
function uploadGambar($file, $current_host)
{
    ...
    return ['success' => false, 'message' => 'Format file salah.'];
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $sql_img = "";
        $params = [$nama_produk, $deskripsi, $origin, $altitude, $variety, $process, $notes, $link];
        $types = "ssssssss";
        ...
        if (isset($_FILES['gambar_url']) && $_FILES['gambar_url']['error'] == 0) {
            ...
            if ($res['success']) {
                ...
                if ($dOld && !empty($dOld['gambar_url'])) {
                    $oldName = basename($dOld['gambar_url']);
                    $oldPath = dirname(__DIR__, 3) . '/assets/img/produk/' . $oldName;
        ...
        $sql = "UPDATE produk SET nama_produk=?, deskripsi=?, origin=?, altitude=?, variety=?, process=?, notes=?, link=? $sql_img WHERE id_produk=?";
        ...
        if (isset($_POST['varian_id'])) {
            ...
            for ($i = 0; $i < count($ids); $i++) {
                $v_id = $ids[$i];
                $sz = (int)$sizes[$i];
                $gr = (!empty($grinds[$i]) && $grinds[$i] !== 'N/A') ? (string)$grinds[$i] : NULL;
                $pr = (int)$hargas[$i];
                $st = (int)$stoks[$i];

                if ($v_id == 'new') {
                    $stmt_ins->bind_param("isiii", $id_produk, $gr, $sz, $st, $pr);
                    $stmt_ins->execute();
                } else {
