...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (addForm) {
    addForm.addEventListener("submit", (e) =>
      handleStatusSubmit(e, "action/store.php", "Status berhasil ditambahkan!")
    );
  ...
  if (updateForm) {
    updateForm.addEventListener("submit", (e) =>
      handleStatusSubmit(e, "action/update.php", "Status berhasil diperbarui!")
    );
    initFormAutoSave(updateForm);
  }

  // Search - kolom [1]=Nama Status
  initTableSearch('searchStatus', '.status-table tbody', [1]);
...
async function handleStatusSubmit(e, url, successMessage) {
  ...
  showLoading("Menyimpan data Status...");
...
async function confirmDelete(id) {
  ...
  if (!confirm("⚠️ Yakin ingin menghapus Status ini?\n\nData yang dihapus tidak dapat dikembalikan!")) {
    return;
  }

  showLoading("Menghapus Status...");
  ...
  try {
    ...
    if (result.success) {
      showNotification("Status berhasil dihapus!", "success");
      setTimeout(() => window.location.reload(), 1000);
    } else {
      showNotification(result.message || "Gagal menghapus Status", "error");
    }
  } catch (error) {
    hideLoading();
    console.error("Error:", error);
