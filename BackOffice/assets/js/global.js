...
function initVariantForm(addBtnId, containerId, templateId) {
  ...
  addVariantBtn.addEventListener("click", function () {
    ...
    const firstDeleteBtn = variantContainer.querySelector(
      ".btn-danger[disabled]"
    );
    if (firstDeleteBtn) {
  ...
  if (variantIdInput && variantIdInput.value !== "new") {
    ...
    if (container && container.children.length > minRows) {
      ...
      if (container.children.length === minRows) {
        const lastDeleteBtn = container.querySelector(
          '.btn-danger:not([style*="display: none"])'
        );
        if (lastDeleteBtn) {
      ...
      if (typeof showNotification === "function") {
        showNotification(
          "Minimal harus ada " + minRows + " varian produk.",
          "warning"
        );
      } else {
...
/* ============================================
   SISTEM PREVIEW GAMBAR (WAJIB ADA)
   ============================================ */
...
function handleImagePreview(input, previewId) {
  ...
  if (file) {
    const reader = new FileReader();
    reader.onload = function (e) {
      // Timpa isi container dengan gambar baru
      previewContainer.innerHTML = `
                <img src="${e.target.result}" 
                     style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;">
                <div style="position: absolute; bottom: 0; left: 0; right: 0; background: rgba(0,0,0,0.5); color: white; text-align: center; font-size: 12px; padding: 4px;">
                    Gambar Baru Dipilih
                </div>
            `;
    };
    ...
    Array.from(input.files).forEach((file) => {
      ...
      reader.onload = function (e) {
        ...
        wrapper.style.cssText =
          "width: 80px; height: 80px; position: relative; border-radius: 6px; overflow: hidden; border: 1px solid #ddd;";
