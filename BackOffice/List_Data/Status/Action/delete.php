...
try {
    ...
    if ($result_check->num_rows == 0) {
        throw new Exception("Data Status tidak ditemukan!");
    }
    $stmt_check->close();

    $stmt_delete = $conn->prepare("DELETE FROM status WHERE id_status = ?");
    $stmt_delete->bind_param("i", $id);

    if (!$stmt_delete->execute()) {
        throw new Exception("Gagal menghapus data Status");
    }
    $stmt_delete->close();
    $conn->close();
